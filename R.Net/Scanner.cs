...
public class Scanner {
	...
	static Scanner() {
		...
		for (int i = 49; i <= 57; ++i) start[i] = 3;
		for (int i = 39; i <= 39; ++i) start[i] = 11;
		for (int i = 34; i <= 34; ++i) start[i] = 12;
		for (int i = 96; i <= 96; ++i) start[i] = 13;
		for (int i = 37; i <= 37; ++i) start[i] = 15;
		start[46] = 54; 
		start[48] = 18; 
		start[10] = 25; 
		start[59] = 26; 
	...
	void CheckLiteral() {
		switch (t.val) {
			...
			case "repeat": t.kind = 13; break;
			case "for": t.kind = 14; break;
			case "in": t.kind = 15; break;
			case "break": t.kind = 16; break;
			case "next": t.kind = 17; break;
			case "NULL": t.kind = 50; break;
			case "NA": t.kind = 51; break;
			case "NA_integer_": t.kind = 52; break;
			case "NA_real_": t.kind = 53; break;
			case "NA_complex_": t.kind = 54; break;
			case "NA_character_": t.kind = 55; break;
			case "TRUE": t.kind = 56; break;
			case "FALSE": t.kind = 57; break;
			case "Inf": t.kind = 58; break;
			case "NaN": t.kind = 59; break;
			default: break;
		}
	}
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 3:
				...
				else if (ch == 'L' || ch == 'i') {AddCh(); goto case 10;}
			...
			case 11:
				...
				else if (ch <= '&' || ch >= '(' && ch <= '[' || ch >= ']' && ch <= 65535) {AddCh(); goto case 11;}
				else {goto case 0;}
			case 12:
				if (ch == '"') {AddCh(); goto case 14;}
				else if (ch == 92) {AddCh(); goto case 20;}
				else if (ch <= '!' || ch >= '#' && ch <= '[' || ch >= ']' && ch <= 65535) {AddCh(); goto case 12;}
				else {goto case 0;}
			case 13:
				if (ch == '`') {AddCh(); goto case 14;}
				else if (ch == 92) {AddCh(); goto case 21;}
				else if (ch <= '[' || ch >= ']' && ch <= '_' || ch >= 'a' && ch <= 65535) {AddCh(); goto case 13;}
				else {goto case 0;}
			case 14:
				{t.kind = 3; break;}
			case 15:
				if (ch <= '$' || ch >= '&' && ch <= 65535) {AddCh(); goto case 16;}
				else {goto case 0;}
			case 16:
				if (ch == '%') {AddCh(); goto case 17;}
				else if (ch <= '$' || ch >= '&' && ch <= 65535) {AddCh(); goto case 16;}
				else {goto case 0;}
			case 17:
				{t.kind = 4; break;}
			case 18:
